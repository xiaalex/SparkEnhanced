...
class SparkContext(config: SparkConf) extends Logging with ExecutorAllocationClient {
  ...
  def delimitFile(
      ...
      delimiter: String,
      schema: Seq[String],
      conf: Configuration = hadoopConfiguration): RDD[DataBlock] = withScope {
    assertNotStopped()
    val rdd = newAPIHadoopFile[LongWritable, Text, NewTextInputFormat](
      path,
      classOf[NewTextInputFormat],
      classOf[LongWritable],
      classOf[Text],
      conf = conf)
    val data = rdd.map { case (k, v) =>
      new DataBlock(delimiter, schema, v.toString)
    }
    data
  }
