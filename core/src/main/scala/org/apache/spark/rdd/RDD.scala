...
import org.apache.spark.util.random.{BernoulliSampler, PoissonSampler, BernoulliCellSampler,
  SamplingUtils}
import org.apache.spark.util.TabIterator
...
abstract class RDD[T: ClassTag](
  ...
  def dmap[U: ClassTag](index: Int, f: T => U): RDD[U] = {
    val cleanF = sc.clean(f)
    new MapPartitionsRDD[U, T](this, (context, pid, iter) => {
      new TabIterator(index, iter).map(cleanF) }
    )
  }
  ...
  def dfilter(index: Int, f: T => Boolean): RDD[T] = {
    val cleanF = sc.clean(f)
    new MapPartitionsRDD[T, T](
      this,
      (context, pid, iter) => new TabIterator(index, iter).filter(cleanF),
      preservesPartitioning = true)
  }
